...
An Electron desktop app for running and monitoring multiple interactive `claude` CLI sessions side by side. Three panes: session list (left, colored status dots), the selected session's live terminal (center), and a git manager — stage/unstage/commit/push for one shared repo (right).
