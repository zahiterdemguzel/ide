...
//
//   /p/<room>/<port>/…      an entry URL from the phone's Ports tab: 302 to the
//                           site root and remember room+port in a cookie
//   Cookie: _idetunnel=…    anything below that root — pages, assets, the HMR
//                           websocket — spliced raw to that room's desktop
...
// A room id is ours (a uuid) and a port is a port; anything else is someone
// poking at the relay, and must not reach a desktop.
...
function route({ target, headers }) {
  ...
  if (entry) {
    const [, room, port, rest] = entry;
    if (!isRoom(room) || !isPort(port)) return { kind: 'deny' };
    const location = (rest || '/') + (query ? `?${query}` : '');
    return {
      kind: 'entry',
      room,
      port: Number(port),
      location,
      setCookie: `${TUNNEL_COOKIE}=${room}.${port}; HttpOnly; Path=/; SameSite=Lax`,
    };
  }
  ...
  if (cookie) {
    ...
    const room = cookie.slice(0, dot);
    const port = cookie.slice(dot + 1);
    if (isRoom(room) && isPort(port)) return { kind: 'tunnel', room, port: Number(port) };
    return { kind: 'deny' };
  }
...
module.exports = { parseHead, route, TUNNEL_COOKIE, MAX_HEAD };
