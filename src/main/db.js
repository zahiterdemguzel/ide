...
// Opens SQLite-family database files in-process via sql.js (SQLite compiled to
// WebAssembly — pure JS, no native build step, matching this app's constraint).
// sql.js loads the whole file into memory; we keep that Database per open file in
// `open` so unsaved edits survive switching files (mirroring the text/sheet
// editors), and export()+write it back on Save. The pure SQL building / identity
// logic lives in db-sql.js; this module is the sql.js + filesystem shell.
//
// Only SQLite-family files are editable. Other recognized formats (Access,
// DuckDB, …) are reported back so the renderer can name them and explain that
// in-app editing isn't available — never opened blindly.
...
// sql.js is large and pulls in a WASM module, so load it lazily the first time a
// database is opened — app startup never pays for it. locateFile points the
// emscripten glue at the .wasm sitting next to it in node_modules (resolves
// transparently inside the packaged asar too).
...
ipcMain.handle('db-open', (_e, file) => openDb(file));
ipcMain.handle('db-table', (_e, { file, table, page }) => loadTable(file, table, page));
...
ipcMain.handle('db-insert-row', (_e, { file, table, values }) => insertRow(file, table, values));
ipcMain.handle('db-delete-row', (_e, { file, table, rowid, originalRow }) => deleteRow(file, table, rowid, originalRow));
ipcMain.handle('db-query', (_e, { file, sql }) => runQuery(file, sql));
ipcMain.handle('db-save', (_e, file) => saveDb(file));
