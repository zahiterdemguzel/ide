...
// Dev runs (`npm start`) get their own title and a hue-rotated icon
// (scripts/gen-dev-icon.js) so an unpackaged window is never mistaken for the
// installed build sitting next to it in the taskbar.
// icon-dev.png is generated by `npm prestart`, so fall back to the real icon
// when the app is launched some other way (e.g. a bare `electron .`).
const APP_NAME = app.isPackaged ? 'IDE' : 'IDE (dev)';
const iconPath = (name) => path.join(__dirname, '..', '..', 'assets', name);
const ICON = !app.isPackaged && fs.existsSync(iconPath('icon-dev.png'))
...
function setWindowTitle(folderPath) {
  if (win && folderPath) win.setTitle(`${APP_NAME} / ${path.basename(folderPath)}`);
}
