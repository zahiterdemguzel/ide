...
The mobile companion ships separately: `build:android` (`scripts/build-android.mjs`) runs `expo prebuild` on the app in `mobile/` and then a Gradle assemble, leaving the APK at `dist/ide-remote.apk`. It needs a JDK and the Android SDK (`ANDROID_HOME`) locally, so CI does not run it. It defaults to a **fast debug build** (skips R8 minification); pass `--release` (`npm run build:android -- --release`) for a shippable minified APK. Both variants **bundle the JS**: the script patches `debuggableVariants = []` into the generated `app/build.gradle` after every prebuild, because React Native's stock debug variant skips bundling and expects a Metro dev server — installed on a phone it dies with "Unable to load script … index.android.bundle". Don't remove the patch. **The launcher icon is the desktop app's, split dev/prod the same way the desktop window is** (see [architecture.md](architecture.md)'s `window.js` entry): the script runs `scripts/gen-icons.js` first — which derives `mobile/assets/icon.png` and its hue-rotated `icon-dev.png` from `assets/icon.png`, both gitignored — and sets `APP_VARIANT`, which `mobile/app.config.js` reads to choose one (`production` → the real mark, anything else → the dev twin). So a debug APK and a release APK are told apart on a phone holding both. `app.config.js` is a thin wrapper over the still-static `app.json`; it sets no `adaptiveIcon`, because the artwork is full-bleed and Android's adaptive mask would crop a fifth of it away. The prebuild stamp keys on the variant and the icon bytes as well as both config files, so switching debug↔release regenerates `android/` rather than reusing one baked with the other icon. Gradle is tuned for speed: a pinned `GRADLE_USER_HOME` at `<stage>/.gradle-home` (ASCII path — keeps the daemon + build cache warm across runs and off the non-ASCII repo path) holds a generated `gradle.properties` with a 4 GB heap, `parallel`, and `caching`; the assemble runs with `--build-cache --parallel`.
...
- `src/main/instance.js` first splits **dev from packaged**: unpackaged runs (`!app.isPackaged`) redirect the default userData to a `-dev` sibling dir before anything reads a path. Without this, `npm start` and the installed build share `%APPDATA%\<name>` — same Chromium caches *and* the same `remote-config.json` (relay room id), paired devices, and instance registry, so a phone paired with the built app could land on a dev window.
- `src/main/instance.js` then redirects `userData` to a **per-instance profile dir** (`<userData>/instances/<pid>`) before any subsystem reads a path from it, so each instance has its own disposable Chromium cache and singleton lock. The profile is deleted on `quit`. Persistent config (e.g. `recent-folders.json`) stays in the captured `sharedDataDir` so it survives restarts and is common to all instances.
