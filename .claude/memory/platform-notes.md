...
The mobile companion ships separately: `build:android` (`scripts/build-android.mjs`) runs `expo prebuild` on the app in `mobile/` and then a Gradle assemble, leaving the APK at `dist/ide-remote.apk`. It needs a JDK and the Android SDK (`ANDROID_HOME`) locally, so CI does not run it. It defaults to a **fast debug build** (skips R8 minification + Hermes bytecode compilation); pass `--release` (`npm run build:android -- --release`) for a shippable minified APK. Gradle is tuned for speed: a pinned `GRADLE_USER_HOME` at `<stage>/.gradle-home` (ASCII path — keeps the daemon + build cache warm across runs and off the non-ASCII repo path) holds a generated `gradle.properties` with a 4 GB heap, `parallel`, and `caching`; the assemble runs with `--build-cache --parallel`.
