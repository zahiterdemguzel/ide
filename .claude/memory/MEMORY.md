...
- [Voice input](voice-input.md) — dictate-on-hover: while the cursor rests over a session terminal, offline Whisper (sherpa-onnx) transcribes speech and types it into that session. The three privacy guards, the phrase-at-a-pause VAD design, the latency/thread-count trade-off, the hallucination filter, and the install-time model fetch.
