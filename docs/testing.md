...
| `src/main/claude-install.js` | `src/main/claude.js`, `src/main/sessions.js` | `installGuide(platform)` — the per-platform Claude Code setup commands for the [setup wizard](architecture.md#claude-code-setup-gate): `installCommand()` (native installer per OS, plus the quoted completion marker the wizard watches for — asserted not to appear unquoted so the shell's own command-echo can't trip it) and `authCommand()` (runs `claude` with the PATH-refresh per OS). `claude.js`'s `claudeAvailable()` runs the actual `claude --version` probe and sessions.js wires the `check-claude` IPC. |
