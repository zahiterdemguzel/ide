...
| `src/main/claude-install.js` | `src/main/claude.js`, `src/main/sessions.js` | `installGuide(platform)` — the per-platform commands the [setup wizard](architecture.md#claude-code-setup-gate) runs: the native installer (`install`), the same plus `; exit` so the install shell terminates for completion-detection (`installTerminal`), and the sign-in command (`auth`: bare `claude` on POSIX, PATH-refresh + `claude` on Windows). `claude.js`'s `claudeAvailable()`/`claudeAuthed()` run the real `claude --version` probe and credentials-file read; sessions.js wires the `check-claude`/`check-claude-auth` IPC. |
