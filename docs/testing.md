...
| `src/main/claude-install.js` | `src/main/claude.js`, `src/main/sessions.js` | `installGuide(platform)` — the per-platform Claude Code install commands (PowerShell installer on Windows, shell installer on macOS/Linux; npm fallback **Linux-only**, `null` on macOS/Windows where the native installer self-suffices) and `withAuth()` (chains `claude` after install with the PATH-refresh per OS) for the [setup wizard](architecture.md#claude-code-setup-gate). `claude.js`'s `claudeAvailable()` runs the actual `claude --version` probe and sessions.js wires the `check-claude` IPC. |
