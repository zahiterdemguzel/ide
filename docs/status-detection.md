...
The dot state is saved with the session snapshot so it survives closing the app. As the hook server maps each event to a state, it also calls `sessions.setSessionState()` to record the live value on the session record (and a successful per-session commit marks it `pushed`); `serializeSession()` then writes it to `sessions.json`. Only the two **settled** states survive verbatim — `completed` (green) and `pushed` (purple) — because a restored session's Claude process can't outlive the app. Every other state (`idle`/`working`/`needs-input`) is collapsed by `persistedState()` to **`interrupted`** (red) on disk, so a session caught mid-flight reopens red rather than pretending an in-flight state is still live. On startup `restoreSessionRow()` paints the dot from the persisted state; selecting the session resumes its Claude process, which then drives the dot live again. `persistedState()` lives in the Electron-free `session-persist.js` and is unit-tested.
