...
Two pieces of state live *inside* the disposable profile by default but must survive it: the inline browser's login cookies, and the renderer's `localStorage` settings (theme, locale, panels, agent-model defaults — see [settings.md](settings.md)) — Chromium backs the latter with a `Local Storage` folder directly under `userData`. `instance.js` junctions both folders (`Partitions/browser` and `Local Storage`) from the per-instance dir to a shared folder under `sharedDataDir`, so they read/write through to a persistent location instead of being wiped on quit. The quit handler removes each junction (`rmdirSync`, which only deletes the reparse point) **before** the recursive delete of the per-instance dir, so the shared targets are never swept up. Concurrent instances sharing these folders' on-disk storage is an accepted trade-off — writes are infrequent enough that simultaneous access from two windows is not a practical concern.
